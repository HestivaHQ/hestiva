...
A large FAQ embedded only on the homepage would make the homepage unnecessarily long and would create pressure to duplicate answers elsewhere. Independent FAQ copies would risk divergence between visible content and structured data.
...
4. The dedicated page uses the same dataset for visible answers and `FAQPage` structured data.
...
- Search engines can crawl the FAQ as a normal indexable page and receive matching FAQ structured data.
