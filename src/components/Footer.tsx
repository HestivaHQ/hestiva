    ...
    <footer className="border-t border-[#C9A45B]/30 bg-[#3B0F1A] text-[#F5F1E8]">
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-9 sm:grid-cols-2 lg:grid-cols-[1.25fr_1fr_1fr] lg:gap-10">
        <div>
          ...
          <p className="mt-2 max-w-xs text-sm leading-6 text-[#F5F1E8]/65">Thoughtful residential cleaning for beautifully cared-for homes.</p>
        </div>
        <nav aria-label="Footer navigation">
          <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-[#D8B970]">Explore</h2>
          <ul className="mt-4 grid gap-2 text-sm">
            {navigation.map((item) => (<li key={item.to}><Link to={item.to} className={linkStyles}>{item.label}</Link></li>))}
          </ul>
        </nav>
        <div>
          ...
          <ul className="mt-4 grid gap-2 text-sm">
            ...
            <li><a href="mailto:info@hestiva.co.za" className={`${linkStyles} break-all`}>info@hestiva.co.za</a></li>
            <li><a href="mailto:quotes@hestiva.co.za" className={`${linkStyles} break-all`}>quotes@hestiva.co.za</a></li>
          </ul>
        </div>
      </div>
