      ...
      <main>
        ...
        <section aria-labelledby="purpose-heading" className="px-6 py-20 md:py-28">
          <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-24">
            <p className="pt-2 text-xs font-semibold uppercase tracking-[0.28em] text-[#9A742E]">
              Our purpose
            </p>
            <div>
              <h2
                id="purpose-heading"
                className="max-w-3xl text-3xl font-semibold tracking-tight text-[#5A1425] md:text-5xl"
              >
        ...
        <section aria-labelledby="principles-heading" className="bg-white/70 px-6 py-20 md:py-28">
            ...
            <div className="mt-14 grid gap-x-10 gap-y-12 sm:grid-cols-2 lg:grid-cols-4">
              {principles.map(({ icon: Icon, title, description }) => (
                <article key={title} className="border-t border-[#C9A45B]/45 pt-7">
                  ...
                  <h3 className="mt-5 text-xl font-semibold text-[#5A1425]">{title}</h3>
                  <p className="mt-3 text-sm leading-7 text-[#695E59]">{description}</p>
                </article>
              ))}
            </div>
          </div>
            ...
            <ol className="relative mt-16 grid gap-12 lg:grid-cols-3 lg:gap-14">
              ...
              {approach.map((step) => (
                <li key={step.title} className="relative text-center">
                  <span className="relative mx-auto flex h-12 w-12 items-center justify-center rounded-full border border-[#C9A45B] bg-[#FBF7EF] text-xs font-semibold tracking-[0.12em] text-[#8A6729]">
                    {step.number}
                  </span>
                  <h3 className="mt-6 text-2xl font-semibold text-[#5A1425]">{step.title}</h3>
        ...
        <section aria-labelledby="trust-heading" className="bg-[#F3EBDD] px-6 py-20 md:py-28">
          <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[0.85fr_1.15fr] lg:gap-24">
            <div>
              <ShieldCheck
                aria-hidden="true"
                className="h-9 w-9 text-[#9A742E]"
                strokeWidth={1.4}
              />
              <p className="mt-6 text-xs font-semibold uppercase tracking-[0.28em] text-[#9A742E]">
                Trust in the home
              </p>
              <h2
                id="trust-heading"
        ...
        <section aria-labelledby="serve-heading" className="px-6 py-20 md:py-28">
          <div className="mx-auto max-w-7xl">
            <div className="grid gap-10 lg:grid-cols-2 lg:items-end lg:gap-24">
              ...
              <p className="max-w-xl text-lg leading-8 text-[#695E59]">
            ...
            <ul className="mt-12 grid gap-x-10 border-y border-[#C9A45B]/30 py-3 sm:grid-cols-2 lg:grid-cols-3">
              {households.map((household) => (
                <li
                  key={household}
                  className="flex items-center gap-3 border-b border-[#C9A45B]/20 py-5 text-[#514946] last:border-b-0 sm:[&:nth-last-child(-n+2)]:border-b-0 lg:[&:nth-last-child(-n+3)]:border-b-0"
                >
                  <span aria-hidden="true" className="h-1.5 w-1.5 rounded-full bg-[#9A742E]" />
                  {household}
                </li>
              ))}
            </ul>
        ...
        <section className="border-t border-[#C9A45B]/20 bg-[#F7F0E3] px-6 py-20 text-center md:py-28">
          <div className="mx-auto max-w-3xl">
            ...
            <div className="mt-10 flex flex-col items-center justify-center gap-3 text-sm text-[#695E59] sm:flex-row sm:gap-6">
              <a
                href="mailto:quotes@hestiva.co.za"
                className="rounded-sm underline decoration-[#C9A45B] underline-offset-4 hover:text-[#5A1425] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A45B]"
              >
  ...
  return (
    <div>
      <div className="flex items-center gap-3">
        {icon ? (
          <span className="text-[#9A742E] [&>svg]:h-5 [&>svg]:w-5" aria-hidden="true">
            {icon}
          </span>
        ) : null}
        <h3 className="font-semibold text-[#5A1425]">{title}</h3>
      </div>
      <p className="mt-2 text-sm leading-6 text-[#695E59]">{children}</p>
    </div>
  );
}
