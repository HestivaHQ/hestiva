    ...
    <div className="min-h-screen bg-[#FBF7EF] text-[#322B2A]">
      ...
      <main>
        <header className="relative overflow-hidden border-b border-[#C9A45B]/25 bg-[#F7F0E3] px-6 pb-24 pt-36 md:pb-32 md:pt-44">
          <div
            aria-hidden="true"
            className="absolute -right-28 top-16 h-96 w-96 rounded-full border border-[#C9A45B]/20"
          />
          <div className="relative mx-auto max-w-7xl">
            ...
            <p className="mt-7 max-w-3xl text-lg leading-8 text-[#5F4B46] md:text-xl">
              Practical answers about choosing a service, requesting a quote, preparing your home,
              managing a booking and what to do after a clean.
            </p>
          </div>
        </header>
        ...
        <div className="mx-auto max-w-6xl px-6 py-20 md:py-28">
          <nav
            ...
            className="mb-20 rounded-2xl border border-[#E7DCC9] bg-[#FFFDF8] p-6 md:p-8"
          >
            <p className="text-xs font-semibold uppercase tracking-[0.24em] text-[#9A7132]">
              Browse by topic
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              {faqCategories.map((category, index) => (
                <a
                  key={category.title}
                  href={`#faq-category-${index + 1}`}
                  className="rounded-full border border-[#C9A45B]/35 bg-[#F8F3E8] px-4 py-2 text-sm font-semibold text-[#3B0F1A] transition hover:border-[#9A7132] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A45B]"
                >
                  {category.title}
                </a>
              ))}
          ...
          <div className="space-y-20">
            {faqCategories.map((category, categoryIndex) => (
              <section
                key={category.title}
                id={`faq-category-${categoryIndex + 1}`}
                aria-labelledby={`faq-category-heading-${categoryIndex + 1}`}
              ...
              >
                <div className="grid gap-8 lg:grid-cols-[0.34fr_0.66fr] lg:gap-16">
                  <div>
                    ...
                    <h2
                      id={`faq-category-heading-${categoryIndex + 1}`}
                      className="mt-3 text-3xl font-semibold tracking-tight text-[#3B0F1A] md:text-4xl"
                    >
                      {category.title}
                    </h2>
        ...
        <section className="border-t border-[#C9A45B]/20 bg-[#3B0F1A] px-6 py-20 text-center text-[#F5F1E8] md:py-24">
          <div className="mx-auto max-w-3xl">
            ...
            <p className="mx-auto mt-5 max-w-2xl leading-7 text-[#F5F1E8]/75">
              If your situation is not covered here, contact Homent or tell us about your home in a
              quote request and we will respond with the information relevant to your service.
            </p>
            <div className="mt-8 flex flex-col justify-center gap-4 sm:flex-row">
              <Link
                to="/quote"
                className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-[#C9A45B] px-7 py-3 text-sm font-semibold uppercase tracking-[0.12em] text-[#3B0F1A] transition hover:bg-[#D8B970] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#F5F1E8]"
              >
                Request a Quote
              ...
              <Link
                to="/contact"
                className="inline-flex min-h-12 items-center justify-center rounded-md border border-[#C9A45B]/60 px-7 py-3 text-sm font-semibold uppercase tracking-[0.12em] text-[#F5F1E8] transition hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#C9A45B]"
              >
                Contact Homent
              </Link>
            </div>
